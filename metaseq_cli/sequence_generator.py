...
from metaseq import search, options, utils, tasks
...
def main(cfg: DictConfig) -> None:
    ...
    task.load_dataset("generation")
    itr_valid = trainer.get_valid_iterator("generation").next_epoch_itr(shuffle=False, set_dataset_epoch=False)

    sample = next(itr_valid)
    sample = _move_to(sample, torch.cuda.current_device())

    sequence_generator = SequenceGenerator(
        [model], task.dictionary, max_len_b=sample['net_input']['src_tokens'].shape[-1], beam_size=5)
    a = sequence_generator.generate([model], sample, bos_token=task.dictionary.bos_index)
    print('a')
    pass
