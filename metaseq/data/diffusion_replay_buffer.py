...
class BufferStack(list):
    ...
    def append(self, item):
        ...
        #     return self.append_stack(item)
            ...
            raise NotImplemented(f"unkown eviction_policy: {self.eviction_policy}")
...
class DiffusionReplayBufferDataset(BaseWrapperDataset):
    def __init__(
        ...
        self.replay_buffer = defaultdict(BufferStack)
...
class StreamingDiffusionTokenBlockDatasetWithReplayBuffer(StreamingTokenBlockDataset):
    def __init__(
        ...
        self.diffusion_step_probabilities = diffusion_step_probabilities
        buffer_stack = functools.partial(BufferStack, eviction_policy=eviction_policy, max_buffer_size=max_buffer_size)
                ...
                while T != 0:
                    ...
                    yield {
                        ...
                        "token_embeddings": (item_diff["probs"].to(device)
                                             @ self.embedding_module.weight).cpu(),
                        "split": self.split,
                    }
                    T = self.sample_diffusion_step_existing()
                if item is None:
